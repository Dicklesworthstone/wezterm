...
function fixCopyOnlyUserSelectable() {
  buttonsToFix = document.querySelectorAll(".language-console button.md-code__button");
  if (buttonsToFix.length) console.log("Fixing copy-to-clipboard text of console code-blocks.");
...
function extractUserSelectable(selector) {
  ...
  element.childNodes.forEach((child) => {
    if (child instanceof Element) {
      ...
      if (
        s.getPropertyValue("user-select") == "none" ||
